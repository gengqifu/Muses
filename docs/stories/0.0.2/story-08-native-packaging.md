...
- ✅ [5] Flutter 层适配：插件改为引用原生包（path/maven/Pods），示例调整依赖方式；发布脚本更新。（适配方案见 `docs/plans/story08-flutter-adapter.md`；Android 已支持调试阶段通过 settings.gradle 引入本地 `native/android-visualization` 模块，发布阶段可切换 maven 依赖；iOS 待接入 XCFramework 后跟进）
