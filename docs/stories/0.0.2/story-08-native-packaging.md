...
- ✖️ [5] Flutter 层适配：插件改为引用原生包（path/maven/Pods），示例调整依赖方式；发布脚本更新。（需同时支持 Android/iOS 调试使用本地原生模块、发布使用 maven/Pod/SPM 原生库；Android 本地 module 已接入，iOS 待 XCFramework 本地/Pod path 接入后再标记完成；参考 `docs/plans/story08-flutter-adapter.md`）
