...
- **Audio Core (C/C++) Agent**：负责音频解码（平台解码器输出 PCM；当前 native 为桩）、重采样、缓冲、PCM 队列；实现时域/频域处理（窗口化、FFT，默认 KissFFT）；提供零拷贝/低拷贝数据面向 UI 层。
...
- **DevOps Agent**：CI 配置（格式化、静态检查、单测、集成测）；构建产物分发；依赖管理与缓存（Flutter/CMake）；环境维护（Flutter SDK 路径/权限、可写 HOME 目录、Android SDK/NDK/CMake 版本一致性）。

## 3. TDD 开发准则
- Story 编写与执行：遵循 TDD 模板（`docs/stories/story-template.md`），先写失败用例（含验收标准/DoD），再实现与回归。
...
- **C/C++ 音频核心**
  - 解码：平台解码器输出统一的 PCM（float32/16bit 可配置），native 桩用于测试/接口占位。
  - 缓冲：环形缓冲 + 双缓冲输出，保证实时性。
...
- CMake 构建 C++ 核心；解码由平台 `AVAudioEngine` / `MediaCodec` 负责，native 层只保留缓冲/FFT/时钟。
- FFT：KissFFT（轻量）。
...
## 9. 风险与缓解
- 移动端解码由平台承担：避免第三方许可/包体风险，CI 仅缓存 Flutter/NDK/CMake。
