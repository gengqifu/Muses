...
## 1. 背景与目标
- 为移动端（iOS/Android）提供音频播放与**实时可视化**（时域波形、频域谱）的体验，支持本地文件与流式来源。
- 可视化目的：让用户/研发能直观观察节奏与能量分布、辅助精准 seek/编辑、并用于调试解码/重采样/时钟同步（发现失真、XRuns、弱网卡顿）。
- 目标：低延迟、稳定帧率，音频与可视化同步；提供基础交互（播放/暂停/拖动/缩放）；弱网与长时间播放保持稳定。
...
- 跨平台音频核心：Android/iOS 共用同一套 C/C++ 算法与音频处理库，通过平台桥接层暴露统一接口，默认使用平台解码器输出 PCM。
- 扩展（非 MVP，规划项）
  - 立体声通道分离显示、能量峰值指示。
  - 录音输入可视化（环回/麦克风）。
  - 波形截图/导出。
...
- 音频核心：C/C++，由平台解码器输出 PCM；环形缓冲、窗口化 + FFT（KissFFT）；核心算法库跨平台通用，仅在平台层做少量 IO/权限适配。
- 构建：CMake 管理原生核心；Flutter 构建移动端包。

## 9. 依赖与外部接口
- 第三方库：KissFFT（FFT），解码依赖平台能力（ExoPlayer/AVFoundation）。
- 系统能力：文件访问、网络请求、音频输出、前后台生命周期。
- 可选：日志/埋点上报 SDK（MVP 可本地存储或调试输出）。

## 10. 风险与缓解
- 平台解码兼容性：关注不同格式/编解码器在系统层支持度，必要时明确不支持列表并降级提示。
